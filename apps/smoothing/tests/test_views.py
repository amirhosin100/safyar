...
class TestBranchView:
    ...
    def test_correct_create(self, api_client, owner_user):
        ...
        assert response.data["smoothing"] == owner_user.branch.smoothing.pk
    ...
    def test_create_with_admin_user(self, api_client, admin_user):
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["smoothing"] == admin_user.branch.smoothing.pk
    ...
    def test_create_superuser_is_scoped_to_own_smoothing(self, api_client, super_user):
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["smoothing"] == super_user.branch.smoothing.pk

    def test_create_ignores_client_submitted_smoothing(self, api_client, owner_user):
        other_owner = self._create_owner("1010101010", "09101010101")
        data = self.branch_data
        data["smoothing"] = other_owner.branch.smoothing.pk
    ...
    def test_with_normal_user(self, api_client, normal_user, owner_user):
        ...
        normal_user.branch = owner_user.branch
        normal_user.save()
    ...
    def test_retrieve_owner_own_branch(self, api_client, owner_user):
        api_client.force_authenticate(owner_user)
        response = api_client.get(self.detail_url(owner_user.branch.pk))

        assert response.status_code == status.HTTP_200_OK
        assert response.data["id"] == owner_user.branch.pk
    ...
    def test_retrieve_owner_other_branch_same_smoothing(self, api_client, owner_user):
        ...
        other_branch = self._create_branch(owner_user.branch.smoothing, name="another branch")
        api_client.force_authenticate(owner_user)
    ...
    def test_retrieve_cross_smoothing_returns_404(self, api_client, owner_user):
        other_owner = self._create_owner("2222222222", "09222222222")
        api_client.force_authenticate(owner_user)

        response = api_client.get(self.detail_url(other_owner.branch.pk))

        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_retrieve_admin_with_allowed_branch(self, api_client, admin_user):
        admin_user.allowed_branches.add(admin_user.branch)
        api_client.force_authenticate(admin_user)

        response = api_client.get(self.detail_url(admin_user.branch.pk))
    ...
    def test_retrieve_superuser_without_allowed_branches_forbidden(self, api_client, super_user):
        api_client.force_authenticate(super_user)
        response = api_client.get(self.detail_url(super_user.branch.pk))
    ...
    def test_retrieve_unauthenticated(self, client, owner_user):
        response = client.get(self.detail_url(owner_user.branch.pk))
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    # ---------------- list ----------------

    def test_list_filters_by_smoothing(self, api_client, owner_user):
        other_owner = self._create_owner("3333333333", "09333333333")
        api_client.force_authenticate(owner_user)
        ...
        assert owner_user.branch.pk in returned_ids
        assert other_owner.branch.pk not in returned_ids

    def test_list_superuser_scoped_to_own_smoothing(self, api_client, super_user):
        other_owner = self._create_owner("3434343434", "09343434343")
        api_client.force_authenticate(super_user)
        ...
        assert response.status_code == status.HTTP_200_OK
        returned_ids = {branch["id"] for branch in response.data}
        assert super_user.branch.pk in returned_ids
        assert other_owner.branch.pk not in returned_ids
    ...
    def test_list_admin_ignores_allowed_branches(self, api_client, admin_user):
        ...
        """
        self._create_branch(admin_user.branch.smoothing, name="not allowed")
        api_client.force_authenticate(admin_user)
    ...
    def test_update_owner(self, api_client, owner_user):
        api_client.force_authenticate(owner_user)
        response = api_client.patch(self.detail_url(owner_user.branch.pk), data={"name": "updated name"})

        assert response.status_code == status.HTTP_200_OK
        assert response.data["name"] == "updated name"

    def test_update_smoothing_field_is_ignored(self, api_client, owner_user):
        other_owner = self._create_owner("4646464646", "09464646464")
        api_client.force_authenticate(owner_user)

        response = api_client.patch(
            self.detail_url(owner_user.branch.pk),
            data={"name": "still mine", "smoothing": other_owner.branch.smoothing.pk},
        )
    ...
    def test_update_cross_smoothing_returns_404(self, api_client, owner_user):
        other_owner = self._create_owner("4444444444", "09444444444")
        api_client.force_authenticate(owner_user)

        response = api_client.patch(self.detail_url(other_owner.branch.pk), data={"name": "hacked"})
    ...
    def test_update_admin_with_allowed_branch(self, api_client, admin_user):
        ...
        response = api_client.patch(self.detail_url(admin_user.branch.pk), data={"name": "updated by admin"})
    ...
    def test_update_admin_without_allowed_branch_forbidden(self, api_client, admin_user):
        ...
        response = api_client.patch(self.detail_url(admin_user.branch.pk), data={"name": "hacked"})

        assert response.status_code == status.HTTP_403_FORBIDDEN

    def test_update_normal_user_forbidden(self, api_client, normal_user, owner_user):
        api_client.force_authenticate(normal_user)
        response = api_client.patch(self.detail_url(owner_user.branch.pk), data={"name": "hacked"})
    ...
    def test_delete_owner(self, api_client, owner_user):
        extra_branch = self._create_branch(owner_user.branch.smoothing)
        api_client.force_authenticate(owner_user)
    ...
    def test_delete_cross_smoothing_returns_404(self, api_client, owner_user):
        other_owner = self._create_owner("5555555555", "09555555555")
        api_client.force_authenticate(owner_user)

        response = api_client.delete(self.detail_url(other_owner.branch.pk))

        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert Branch.objects.filter(pk=other_owner.branch.pk).exists()

    def test_delete_admin_with_allowed_branch(self, api_client, admin_user):
        extra_branch = self._create_branch(admin_user.branch.smoothing)
    ...
    def test_delete_normal_user_forbidden(self, api_client, normal_user, owner_user):
        api_client.force_authenticate(normal_user)
        response = api_client.delete(self.detail_url(owner_user.branch.pk))
...
class TestSmoothingAPIView:
    ...
    def test_get_owner(self, api_client, owner_user):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["id"] == owner_user.branch.smoothing.pk
    ...
    def test_get_admin_without_branch_crashes(self, client):
        ...
        and then crashes the view: `request.user.branch.smoothing` on a None branch.
        """
